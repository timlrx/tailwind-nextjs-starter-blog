...
export function getFiles(type, locale) {
  ...
  let files
  if (locale === 'en-US') {
    files = getAllFilesRecursively(prefixPaths).filter(
      (path) => (path.match(/\./g) || []).length === 1
    )
  } else {
    files = getAllFilesRecursively(prefixPaths).filter((path) => path.includes(locale))
  }
...
// We give locale if locale !== defaultLocale
export async function getFileBySlug(type, slug, locale = '') {
  const mdxPath =
    locale !== '' && fs.existsSync(path.join(root, 'data', type, `${slug}.${locale}.mdx`))
      ? path.join(root, 'data', type, `${slug}.${locale}.mdx`)
      : path.join(root, 'data', type, `${slug}.mdx`)
  const mdPath =
    locale !== '' && fs.existsSync(path.join(root, 'data', type, `${slug}.${locale}.md`))

...
export function getFiles(type, otherLocale = '') {
  ...
  otherLocale === ''
    ...
    : (files = getAllFilesRecursively(prefixPaths).filter((path) => path.includes(otherLocale)))
...
export async function getFileBySlug(type, slug, otherLocale = '') {
  const mdxPath =
    otherLocale !== '' && fs.existsSync(path.join(root, 'data', type, `${slug}.${otherLocale}.mdx`))
      ? path.join(root, 'data', type, `${slug}.${otherLocale}.mdx`)
      : path.join(root, 'data', type, `${slug}.mdx`)
  const mdPath =
    otherLocale !== '' && fs.existsSync(path.join(root, 'data', type, `${slug}.${otherLocale}.md`))

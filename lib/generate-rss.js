...
const generateRssItem = (post, locale, defaultLocale) => `
  <item>
    <guid>${siteMetadata.siteUrl}${defaultLocale === locale ? "" : "/" + locale}/p/${
  post.slug
}</guid>
    <title>${escape(post.title)}</title>
    <link>${siteMetadata.siteUrl}${defaultLocale === locale ? "" : "/" + locale}/p/${
  post.slug
}</link>
...
const generateRss = (posts, locale, defaultLocale, page = "feed.xml") => `
  <rss version="2.0" xmlns:atom="http://www.w3.org/2005/Atom">
    <channel>
      ...
      <link>${siteMetadata.siteUrl}${defaultLocale === locale ? "" : "/" + locale}/p</link>

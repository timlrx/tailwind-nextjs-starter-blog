...
There is a Backend as a Service (**BaaS**), which you can use as a normal backend with API or messaging.
...
BaaS is usually larger than Faas, or a group of FaaS make one BaaS, it's a group of functions that are accessible through some API.
You need to be careful to not make them very big services, if a BaaS took 3 months to develop, you need to assume you will need 9 months to switch it to a microservice.
...
At one of my previous jobs, we utilized a FaaS (Function as a Service) to retrieve files from an FTP server, save them, and process them.  
...
Serverless architecture is very useful for small functionality in which you don't want any cost in deployment, maintenance, and scalability.

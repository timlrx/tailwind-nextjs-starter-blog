...
There are cases where you can apply this pattern but have one storage in that case, it would be **CQS** since we are not segregating Responsibility and just the Command and Query instead.
A diagram of this would look something like this.
![Same storage CQRS](../../static/images/cqrs_same_storage.jpg)

### Real-world scenario

Suppose you have an online store where you can browse items, check out, and confirm your order.
Your system will execute something like in the diagram after you click the purchase button; after the write, the data or the event will trigger a DB sync listener.
![Order command model](../../static/images/order_command_model.jpg)

### Benefits

- Distinguishing between queries and commands.
- Easier to make improvements on DB if the read or write slows down.
...
### External links on this architecture

- [From Microsoft](https://learn.microsoft.com/en-us/azure/architecture/patterns/cqrs?utm_source=pocket_saves)
